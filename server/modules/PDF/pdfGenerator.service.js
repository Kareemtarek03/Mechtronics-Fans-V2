...
function calculateSoundLevels(
  ...
  return {
    ...
    motorInputPower: parseFloat(motorInputPower.toFixed(3)),
    octaveLW,
    octaveLP,
  };
}
  ...
  constructor(x, y) {
    ...
    for (let i = 0; i < x.length - 1; i++) {
      ...
      this.segments.push({
        ...
        xMax: Math.max(x0, x1),
        a,
        b: y0 - a * Math.pow(x0, 3),
      });
    }
  }
  at(xi) {
    for (const s of this.segments)
      if (xi >= s.xMin && xi <= s.xMax) return s.a * Math.pow(xi, 3) + s.b;
    const s =
      this.segments[xi < this.segments[0].xMin ? 0 : this.segments.length - 1];
...
function getCurvePoints(xArr, yArr, mult = 1) {
  ...
  const interp = new PiecewiseCubicInterpolator(
    pairs.map((p) => p.x),
    pairs.map((p) => p.y)
  );
...
export function generateFanDatasheetPDF(fanData, userInput, units) {
  ...
  console.log("userInput in PDF generator:", units);
  console.log("userInput in PDF generator:", units.fanType);
  const doc = new PDFDocument({ size: "A4", margin: 46 });
  const W = doc.page.width,
    H = doc.page.height,
    M = 46; // 46px margin from left & right
  const cW = W - 2 * M;
  const black = "#000000";
  ...
  let motorEff = 0.85;
  if (motor?.effCurve?.length > 0)
    motorEff =
      motor.effCurve.reduce((a, b) => a + b, 0) / motor.effCurve.length;

  const sound = calculateSoundLevels(
    pred.FanInputPowerPred,
    pred.StaticPressurePred,
    motorEff,
    userInput?.SPF || 5,
    1,
    3
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fillColor(black)
    .text(modelNum, M, topMargin);
  doc
    .fontSize(8)
    .font("Helvetica")
    .fillColor(black)
    .text(modelNum.split("-")[0] + " Series", M, topMargin + 15);
  const logoSize = 25;
  const bigLogoSize = 40;
  const fanSize = 150;
  // Fan image
  const fanType = modelNum.split("-")[0];
  const normalFanTypes = ["AF-L", "AF-S", "WF", "ARTF"];
  ...
  const innerdia = impeller.innerDia + "mm" || "12";
  function image() {
    try {
      // Build path: server/modules/PDF/assets/Rander/AF/PAM/8.jpg

      const imagePath = `server/modules/PDF/assets/Rander/${fanType}/${Material}${bladeSymbol}/${numBlades}.jpg`;

      doc.image(imagePath, W - M - fanSize, topMargin, {
        width: fanSize,
        height: fanSize,
        fit: [fanSize, fanSize],
        align: "center",
      ...
      console.error("Failed to load fan image:", error.message);
      // Fallback to placeholder box if image not found
      doc.rect(W - M - 130, 15, 130, 100).stroke(black);
      doc
        .fontSize(6)
        .fillColor("#999")
        .text("(Fan Image)", W - M - 85, 60);
    }
    try {
      const amcaPath = `server/modules/PDF/assets/AMCA.svg`;
      const svgContent = fs.readFileSync(amcaPath, "utf8");
      SVGtoPDF(
        doc,
        svgContent,
        W - M - fanSize + 6 - bigLogoSize,
        fanSize - bigLogoSize + topMargin - 20,
        {
          width: bigLogoSize,
          height: bigLogoSize,
      ...
      console.error("Failed to load AMCA image:", error.message);
      // Fallback to placeholder box if image not found
      doc.rect(W - M - 225, 140, 25, 25).stroke(black);
    }
    try {
      const amcaPath = `server/modules/PDF/assets/EOS.png`;
      doc.image(
        amcaPath,
        W - M - fanSize - logoSize,
        fanSize - logoSize + topMargin,
        {
          width: logoSize,
          height: logoSize,
      ...
      console.error("Failed to load EOS image:", error.message);
      // Fallback to placeholder box if image not found
      doc.rect(W - M - 210, 150, 25, 25).stroke(black);
    }

    if (normalFanTypes.includes(Type)) {
      const normalPath = "server/modules/PDF/assets/Performance/Normal.jpg";
      doc.image(normalPath, W / 2 - M - bigLogoSize, 140, {
        width: (bigLogoSize + 10) * 2,
        height: bigLogoSize + 10,
        fit: [(bigLogoSize + 10) * 2, bigLogoSize + 10],
        align: "center",
        valign: "center",
      });
    } else {
      try {
        const fireLogo = `server/modules/PDF/assets/Fire/Fire.jpg`;
        doc.image(fireLogo, W - M - fanSize - logoSize, topMargin, {
          width: logoSize,
          height: logoSize,
          fit: [logoSize, logoSize],
          align: "center",
        ...
        // Add performance curve headline and notes

        const smokePath = "server/modules/PDF/assets/Performance/smoke.jpg";
        doc.image(smokePath, W / 2 - M - bigLogoSize, 140, {
          width: (bigLogoSize + 10) * 2,
          height: bigLogoSize + 10,
          fit: [(bigLogoSize + 10) * 2, bigLogoSize + 10],
          align: "center",
        ...
        doc.text(
          "Type B Installation",
          W / 2 - M - bigLogoSize + 90,
          fanSize - bigLogoSize + topMargin + 60,
          {
            width: 100,
            align: "left",
          }
        ...
        console.error("Failed to load Smoke image:", error.message);
      }
    }
  }
  function middleText() {
    ...
    doc.text(
      "EN 12101-3 : 2015",
      W / 2 - M - bigLogoSize + 90,
      fanSize - bigLogoSize + topMargin + 50,
      {
        width: 100,
        align: "left",
      }
    ...
    doc.text("Performance Curve", W / 2 - M - bigLogoSize - 10, topMargin, {
      width: 150,
      align: "left",
    });
    ...
    doc.text(
      "Performance Shown in diagrams refer to air density 1.2 kg/m³",
      W / 2 - M - bigLogoSize - 10,
      topMargin + 20,
      {
        width: 160,
    ...
    doc.text(
      "Performance tested is for installation type B free inlet, ducted outlet.",
      W / 2 - M - bigLogoSize - 10,
      topMargin + 40,
      {
        width: 160,
    ...
    doc.text(
      "Power rating (kW) does not include transmission losses.",
      W / 2 - M - bigLogoSize - 10,
      topMargin + 60,
      {
        width: 160,
    ...
    doc.text(
      "Performance ratings do not include the effect of accessories.",
      W / 2 - M - bigLogoSize - 10,
      topMargin + 80,
      {
        width: 160,
  ...
  function bladeImage() {
    try {
      const Logo = `server/modules/PDF/assets/Logo.svg`;
      const svgContent = fs.readFileSync(Logo, "utf8");
      SVGtoPDF(doc, svgContent, M + 60, topMargin + 50, {
        width: 100,
        height: 100,
      ...
      const blade = `server/modules/PDF/assets/Icons/${
        normalFanTypes.includes(Type) ? "Normal" : "Smoke"
      }/${Material}${bladeSymbol}/B-${numBlades}.jpg`;
      const hub = `server/modules/PDF/assets/Icons/${
        normalFanTypes.includes(Type) ? "Normal" : "Smoke"
      }/${Material}${bladeSymbol}/H-${numBlades}.jpg`;
      const dia = `server/modules/PDF/assets/Icons/${
        normalFanTypes.includes(Type) ? "Normal" : "Smoke"
      }/${Material}${bladeSymbol}/D-${numBlades}.jpg`;
      let i = 0;
      for (const filePath of [blade, hub, dia]) {
        const height = topMargin + 30 + i * 55;
        i += 1;
        doc.image(filePath, M, height, {
          width: 50,
          height: 50,
          fit: [50, 50],
          align: "center",
      ...
      const text = ["Nominal Diameter", "Hub Type", "Number of Blades"];
      i = 0;
      doc.fontSize(8).font("Helvetica").fillColor(black);
      const data = [innerdia, hubType, numBlades];
      for (const t of text) {
        const height = topMargin + 42 + i * 55;
        doc.text(t, M + 60, height, {
          width: 100,
          align: "left",
        });
        doc.text(data[i], M + 60, height + 15, {
          width: 100,
          align: "left",
        });
        i += 1;
      ...
      console.error("Failed to load Logo image:", error.message);
    }
  }
  bladeImage();
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fillColor(black)
    .text("Performance Data", M + 5, y + headingPaddingTop);
  const perfTextWidth = doc.widthOfString("Performance Data");
  doc
    .moveTo(M + 5, y + headingPaddingTop + 10)
    .lineTo(M + 5 + perfTextWidth, y + headingPaddingTop + 10)
    .stroke(black);
  ...
  const perfRows = [
    ...
    [
      `- Design Air Flow [${units?.airFlow || "CFM"}]`,
      ":",
      fmt(userInput?.airFlow, 0),
    ],
    [
      `- Design Static Pressure [${units?.pressure || "Pa"}]`,
      ":",
      fmt(pred.StaticPressurePred, 0),
    ],
    ["- Design Fan Input Power [kW]", ":", fmt(pred.FanInputPowerPred, 2)],
    ["- Design Motor Input Power [kW]", ":", fmt(sound.motorInputPower, 2)],
    [
      "- Design Fan Static Efficiency [%]",
      ":",
      fmtPct(pred.FanStaticEfficiencyPred),
    ],
    [
      "- Design Fan Total Efficiency [%]",
      ":",
      fmtPct(pred.FanTotalEfficiencyPred),
    ],
  ...
  perfRows.forEach((r) => {
    doc
      ...
      .font("Helvetica")
      .fillColor(black)
      .text(r[0], M + 5, y + 1, { width: 120, lineBreak: false });
    doc.text(r[1], M + 127, y + 1, { width: 8, lineBreak: false });
    doc.font("Helvetica-Bold").text(r[2], M + 137, y + 1, {
      width: 60,
      align: "right",
      lineBreak: false,
    });
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fillColor(black)
    .text("Fan Data", rightX + 5, ry);
  const fanTextWidth = doc.widthOfString("Fan Data");
  doc
    .moveTo(rightX + 5, ry + 10)
    .lineTo(rightX + 5 + fanTextWidth, ry + 10)
    .stroke(black);
  ...
  fanRows.forEach((r) => {
    doc
      ...
      .font("Helvetica")
      .fillColor(black)
      .text(r[0], rightX + 5, ry + 1, { width: 110, lineBreak: false });
    doc.text(r[1], rightX + 117, ry + 1, { width: 8, lineBreak: false });
    doc.font("Helvetica-Bold").text(r[2], rightX + 127, ry + 1, {
      width: 65,
      align: "right",
      lineBreak: false,
    });
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fillColor(black)
    .text("Motor Data", rightX + 5, ry);
  const motorTextWidth = doc.widthOfString("Motor Data");
  doc
    .moveTo(rightX + 5, ry + 10)
    .lineTo(rightX + 5 + motorTextWidth, ry + 10)
    .stroke(black);
  ry += 10 + headingPaddingBottom;
  ...
  const motorRows = [
    ...
    [
      "- Voltage [V]/Phase/Freq [Hz]",
      ":",
      `${motor.Phase === 3 ? "380" : "220"}/${motor.Phase || "-"}/50`,
    ],
  ...
  motorRows.forEach((r) => {
    doc
      ...
      .font("Helvetica")
      .fillColor(black)
      .text(r[0], rightX + 5, ry + 1, { width: 115, lineBreak: false });
    doc.text(r[1], rightX + 122, ry + 1, { width: 8, lineBreak: false });
    doc.font("Helvetica-Bold").text(r[2], rightX + 132, ry + 1, {
      width: 60,
      align: "right",
      lineBreak: false,
    });
  ...
  // Vertical divider
  doc
    .moveTo(M + colW + 4, dataBoxStartY)
    .lineTo(M + colW + 4, dataBoxEndY)
    .stroke(black);
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fillColor(black)
    .text("Power spectrum of the sound pressure", M + 3, y + 3, {
      lineBreak: false,
    });
  ...
  if (sound.octaveLP.length) {
    ...
    const lpTotalH = Math.max(
      4,
      ((sound.lpA - minV) / (maxV - minV)) * maxBarH
    );
    doc
      .rect(
        barsStartX + 8 * barW + 1,
        lpBarY + maxBarH - lpTotalH,
        barW - 2,
        lpTotalH
      )
      .fill(violet);
  ...
  doc
    ...
    .lineTo(M + spectrumW - 2, tableY)
    .stroke();
  doc
    .moveTo(tableX, tableY + tableRowH)
    .lineTo(M + spectrumW - 2, tableY + tableRowH)
    .stroke();
  doc
    .moveTo(tableX, tableY + 2 * tableRowH)
    .lineTo(M + spectrumW - 2, tableY + 2 * tableRowH)
    .stroke();
  // Vertical lines for each column
  doc
    .moveTo(tableX, tableY)
    .lineTo(tableX, tableY + 2 * tableRowH)
    .stroke();
  doc
    .moveTo(tableX + labelColW, tableY)
    .lineTo(tableX + labelColW, tableY + 2 * tableRowH)
    .stroke();
  for (let i = 1; i <= numBars; i++) {
    doc
      .moveTo(barsStartX + i * barW, tableY)
      .lineTo(barsStartX + i * barW, tableY + 2 * tableRowH)
      .stroke();
  }
  ...
  bands.forEach((b, i) =>
    doc.text(b, barsStartX + i * barW, tableY + 2, {
      width: barW,
      align: "center",
      lineBreak: false,
    })
  );
  doc.fillColor(violet).text("LP (A)", barsStartX + 8 * barW, tableY + 2, {
    width: barW,
    align: "center",
    lineBreak: false,
  });
  ...
  doc.fillColor(black);
  sound.octaveLP.forEach((v, i) =>
    doc.text(fmt(v, 1), barsStartX + i * barW, tableY + tableRowH + 2, {
      width: barW,
      align: "center",
      lineBreak: false,
    })
  );
  doc.text(fmt(sound.lpA, 1), barsStartX + 8 * barW, tableY + tableRowH + 2, {
    width: barW,
    align: "center",
    lineBreak: false,
  });
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fillColor(black)
    .text("Power spectrum of the sound power", rwX + 3, y + 3, {
      lineBreak: false,
    });
  ...
  if (sound.octaveLW.length) {
    ...
    const lwTotalH = Math.max(
      4,
      ((sound.lwA - minV) / (maxV - minV)) * maxBarH
    );
    doc
      .rect(
        rwBarsStartX + 8 * barW + 1,
        lpBarY + maxBarH - lwTotalH,
        barW - 2,
        lwTotalH
      )
      .fill(violet);
  ...
  // Horizontal lines
  doc
    .moveTo(rwTableX, tableY)
    .lineTo(rwX + spectrumW - 2, tableY)
    .stroke();
  doc
    .moveTo(rwTableX, tableY + tableRowH)
    .lineTo(rwX + spectrumW - 2, tableY + tableRowH)
    .stroke();
  doc
    .moveTo(rwTableX, tableY + 2 * tableRowH)
    .lineTo(rwX + spectrumW - 2, tableY + 2 * tableRowH)
    .stroke();
  // Vertical lines
  doc
    .moveTo(rwTableX, tableY)
    .lineTo(rwTableX, tableY + 2 * tableRowH)
    .stroke();
  doc
    .moveTo(rwTableX + labelColW, tableY)
    .lineTo(rwTableX + labelColW, tableY + 2 * tableRowH)
    .stroke();
  for (let i = 1; i <= numBars; i++) {
    doc
      .moveTo(rwBarsStartX + i * barW, tableY)
      .lineTo(rwBarsStartX + i * barW, tableY + 2 * tableRowH)
      .stroke();
  }
  ...
  bands.forEach((b, i) =>
    doc.text(b, rwBarsStartX + i * barW, tableY + 2, {
      width: barW,
      align: "center",
      lineBreak: false,
    })
  );
  doc.fillColor(violet).text("LW (A)", rwBarsStartX + 8 * barW, tableY + 2, {
    width: barW,
    align: "center",
    lineBreak: false,
  });
  ...
  // Text values in black
  doc.fillColor(black);
  sound.octaveLW.forEach((v, i) =>
    doc.text(fmt(v, 1), rwBarsStartX + i * barW, tableY + tableRowH + 2, {
      width: barW,
      align: "center",
      lineBreak: false,
    })
  );
  doc.text(fmt(sound.lwA, 1), rwBarsStartX + 8 * barW, tableY + tableRowH + 2, {
    width: barW,
    align: "center",
    lineBreak: false,
  });
  ...
  for (let i = 1; i < 8; i++) {
    ...
    doc
      .moveTo(gx + 30, graphY2)
      .lineTo(gx + 30, graphY2 + graphH)
      .stroke();
  }
  for (let i = 1; i < 10; i++) {
    ...
    doc
      .moveTo(graphX + 30, gy)
      .lineTo(graphX + graphW, gy)
      .stroke();
  }

  // Y-axis labels (left side) - colored
  doc.fontSize(4).font("Helvetica-Bold");
  doc.fillColor(greenColor).text("Eff [%]", M + 3, graphY2 - 12);
  doc.fillColor(redColor).text("Pshaft [kW]", M + 30, graphY2 - 12);
  doc
    .fillColor(blueColor)
    .text(`Ps [${units?.pressure || "Pa"}]`, M + 62, graphY2 - 12);
  // Y-axis scales (only every other line to save space)
  doc.fontSize(4).font("Helvetica");
  for (let i = 0; i <= 10; i += 2) {
    const yPos = graphY2 + graphH - (i / 10) * graphH;
    doc.fillColor(greenColor).text(`${i * 10}%`, M, yPos - 2, {
      width: 14,
      align: "right",
      lineBreak: false,
    });
    doc.fillColor(redColor).text(fmt((i * pwMax) / 10, 1), M + 30, yPos - 2, {
      width: 14,
      align: "right",
      lineBreak: false,
    });
    doc.fillColor(blueColor).text(fmt((i * pMax) / 10, 0), M + 60, yPos - 2, {
      width: 14,
      align: "right",
      lineBreak: false,
    });
  ...
  for (let i = 0; i <= 7; i += 2) {
    ...
    doc.text(fmt(xVal, 0), xPos + 20, graphY2 + graphH + 2, {
      width: 20,
      align: "center",
      lineBreak: false,
    });
  ...
  const drawCurve = (pts, yMin, yMax, color) => {
    ...
    let started = false;
    pts.forEach((pt) => {
      const px = graphX + ((pt.x - xMin) / (xMax - xMin)) * graphW + 30;
  ...
  if (opX && opX >= xMin && opX <= xMax) {
    const opPx = graphX + ((opX - xMin) / (xMax - xMin)) * graphW + 30;

    // Vertical dashed line
    doc.strokeColor("#999").lineWidth(0.5).dash(2, { space: 2 });
    doc
      .moveTo(opPx, graphY2)
      .lineTo(opPx, graphY2 + graphH)
      .stroke();
    doc.undash();
    ...
    if (
      pred.StaticPressurePred &&
      pred.StaticPressurePred >= pMin &&
      pred.StaticPressurePred <= pMax
    ) {
      const opPy =
        graphY2 +
        graphH -
        ((pred.StaticPressurePred - pMin) / (pMax - pMin)) * graphH;
      doc.circle(opPx, opPy, 3).fill(blueColor);
    }
    if (
      pred.FanInputPowerPred &&
      pred.FanInputPowerPred >= pwMin &&
      pred.FanInputPowerPred <= pwMax
    ) {
      const opPy =
        graphY2 +
        graphH -
        ((pred.FanInputPowerPred - pwMin) / (pwMax - pwMin)) * graphH;
    ...
    if (pred.FanTotalEfficiencyPred) {
      ...
      if (effVal >= effMin && effVal <= effMax) {
        const opPy =
          graphY2 + graphH - ((effVal - effMin) / (effMax - effMin)) * graphH;
  ...
  doc
    .moveTo(legendX, legendY2 + 3)
    .lineTo(legendX + 12, legendY2 + 3)
    .stroke();
  doc
    .fillColor(black)
    .text("Ps - Pressure", legendX + 15, legendY2, { lineBreak: false });

  doc.strokeColor(redColor);
  doc
    .moveTo(legendX, legendY2 + 10)
    .lineTo(legendX + 12, legendY2 + 10)
    .stroke();
  doc.text("Pshaft - Power", legendX + 15, legendY2 + 7, { lineBreak: false });

  doc.strokeColor(greenColor);
  doc
    .moveTo(legendX, legendY2 + 17)
    .lineTo(legendX + 12, legendY2 + 17)
    .stroke();
  doc.text("Eff - Efficiency", legendX + 15, legendY2 + 14, {
    lineBreak: false,
  });
